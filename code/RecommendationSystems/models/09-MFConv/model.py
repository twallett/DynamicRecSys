...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, hidden_channels, degree):
        super().__init__()
        self.conv1 = MFConv(in_channels = (-1, -1), out_channels = hidden_channels, max_degree=degree)
        self.conv2 = MFConv(in_channels = (-1, -1), out_channels = hidden_channels, max_degree=degree)
        self.conv3 = MFConv(in_channels = (-1, -1), out_channels = hidden_channels, max_degree=degree)
...
class RecommendationSystem(torch.nn.Module):
    ...
    def __init__(self, data, hidden_channels, degree):
        super().__init__()
        self.encoder = Encoder(hidden_channels = hidden_channels, degree = degree)
